...
async def get_dealer(dealer_id: str, user=Depends(require_roles('owner', 'admin', 'manager', 'sales_rep', 'dealer'))):
    query = _visible_query(user)
...
@router.post('')
async def create_dealer(payload: DealerIn, user=Depends(require_roles('owner', 'admin', 'manager', 'sales_rep'))):
...
async def update_dealer(dealer_id: str, payload: DealerIn, user=Depends(require_roles('owner', 'admin', 'manager', 'sales_rep'))):
    # Reps can only edit their own dealers
    query = _visible_query(user)
    query['dealer_id'] = dealer_id
...
async def delete_dealer(dealer_id: str, user=Depends(require_roles('owner', 'admin'))):
